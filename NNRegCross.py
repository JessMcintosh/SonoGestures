...
featuresPath2 =  os.path.realpath(sys.argv[2])
fingersPath2 = os.path.dirname( os.path.realpath(featuresPath2))
...
for ig in GESTURES:
    ...
    for inu in range(0,N_GESTURES):
        ...
        featurePath2 = os.path.join(featuresPath2, ig, ig+str(inu) + ".txt")
        fingerPath2 = os.path.join(fingersPath2, ig, ig+str(inu) + ".txt")
        features[ig].append( loadFile(featurePath) )
        fingers[ig].append( loadFile(fingerPath).T )
        features2[ig].append( loadFile(featurePath2) )
        fingers2[ig].append( loadFile(fingerPath2).T )
...
for ig in GESTURES:
    for n in range(0,N_GESTURES):
        for f in range(0,N_FINGERS):
            ...
            medianFilter(fingers2[ig][n][f], 15)
...
for n in range (0, N_FINGERS):
    if verbose:
        print "Regression for finger " + str(n)
    ...
    for ig in GESTURES:
        for inu in training:
            ...
            sampleN.extend( features2[ig][inu] ) 
            sampleP.extend( fingers2[ig][inu][n] )
