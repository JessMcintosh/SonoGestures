...
int main(int argc, char** argv)
{
	...
	float buffer;
	while(file >> buffer) features.push_back(buffer);
	
	int width = 13;
	int height = 10;

	Mat fMat = Mat::zeros(width, height, CV_32FC1);


	for(int j = 0; j < height; j++){
		for(int i = 0; i < width; i++){
			fMat.at<float>(i,j) = features.at(i + j * height);
		}
	}
	resize(fMat, fMat, Size(1300,1000), 0, 0, INTER_NEAREST);
	imshow("features", fMat);
	waitKey();
